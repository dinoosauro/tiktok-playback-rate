...
(() => {
    function init() {
        for (const item of document.querySelectorAll(".css-16g1ej4-SectionActionBarContainer")) renderItem(item);
    }
    /**
    ...
    function renderItem(item) {
        ...
        button.classList.add(isDarkThemeEnabled ? "css-67yy18-ButtonActionItem" : "css-1ok4pbl-ButtonActionItem");
        ...
        span.classList.add(isDarkThemeEnabled ? "css-1lguwm0-SpanIconWrapper" : "css-8qpsy4-SpanIconWrapper");
        ...
        strong.classList.add(isDarkThemeEnabled ? "css-1w013xe-StrongText" : "css-vc3yj-StrongText");
        svg.append(path);
        span.append(svg);
        button.append(span, strong);
        button.setAttribute("data-playbackratemodifier", "");
        button.onclick = () => {
            const video = item.closest("article").querySelector("video");
            const playbackRate = video.playbackRate === 1 ? 1.25 : video.playbackRate === 1.25 ? 1.5 : video.playbackRate === 1.5 ? 2 : 1;
            video.playbackRate = playbackRate;
            strong.textContent = `${playbackRate}x`;
        }
        (item.querySelector("[data-e2e=share-icon]") ?? item.querySelector("[data-e2e=undefined-icon]") ?? document.querySelector("[data-e2e=comment-icon]") ?? document.querySelector("[data-e2e=like-icon]")).closest("button").insertAdjacentElement("afterend", button);
    }
    new MutationObserver(() => { // Re-render the buttons if the user changes the theme from Settings
